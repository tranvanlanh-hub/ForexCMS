...
const { createHmac, pbkdf2: pbkdf2Callback, randomBytes, createCipheriv } = require("node:crypto");
...
function totpAtCounter(secret, counter) {
  ...
  const digest = createHmac("sha1", decryptBase32(secret)).update(buffer).digest();
