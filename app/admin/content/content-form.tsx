...
export function ContentForm({ action, error, item, brokers, markets, mediaAssets, saved, warning, templates, categories, topics, csrfToken, storageReady, maxBytes }: ContentFormProps & { csrfToken: string; storageReady: boolean; maxBytes: number; }) {
    const isEditing = Boolean(item);
    const formRef = useRef<HTMLFormElement>(null);
    const [contentType, setContentType] = useState(item?.contentType ?? ContentType.ARTICLE);
    ...
    {warning && <div role="status" className="editor-alert warning">Review saved with notes: {warning}</div>}
    <div className="editor-grid"><div className="editor-main">
      ...
      <section className="editor-panel"><div className="panel-heading"><div><h2>Verdict / summary</h2><p>Required when publishing a broker review. It appears at the top of that public page and can also summarize other content.</p></div><span className="panel-badge">Editorial</span></div><label htmlFor="summary">Verdict / summary<textarea defaultValue={item?.summary ?? ""} id="summary" maxLength={500} name="summary" rows={4} placeholder="State who this is for, main strengths, and important limitations."/></label><p className="field-help">Keep it evidence-led. Detailed analysis belongs in the article body.</p></section>
      ...
      <section className="editor-panel"><div className="panel-heading"><h2>{contentType === ContentType.BROKER_REVIEW ? "Attached brokers" : "Attached brokers (optional)"}</h2></div><div className="broker-checkboxes">{brokers.map(broker => <label key={broker.id}><input type="checkbox" name="brokerIds" value={broker.id} defaultChecked={item?.brokers.some(selected => selected.id === broker.id) ?? false}/><span>{broker.name}<small>{broker.slug}</small></span></label>)}</div><p className="field-help">{contentType === ContentType.BROKER_REVIEW ? "The public review uses the attached broker with lowest editorial priority, then name. Facts and MarketGB assessments use the selected market; sourced pros and cons only render from Broker Facts." : "Used for broker facts and affiliate CTAs. Select the brokers relevant to this article."}</p></section>
    </aside></div>
  </form>;
}
