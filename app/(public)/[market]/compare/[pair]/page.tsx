...
type ComparisonData = NonNullable<
  Awaited<ReturnType<typeof getBrokerComparisonByRoute>>
>;
...
function BrokerFactCell({
  ...
  const facts = groupBrokerFactsByCategory(
    broker.factItems.filter(
      (fact) => fact.category === category && fact.sourceName && fact.sourceUrl,
    ),
  )[0]?.facts;
  ...
  return (
    <ul className="space-y-3">
      {facts.map((fact) => (
        <li className="leading-6" key={fact.id}>
          ...
          {fact.appliesTo ? (
            <span className="text-[#5f6268]"> ({fact.appliesTo})</span>
          ) : null}
          <a
...
function ComparisonSnapshot({
  ...
  const rows = [
    ...
    (item): item is [string, NonNullable<(typeof highlights)["spread"]>] =>
      Boolean(item[1]),
  );
  ...
  return (
    <dl className="mt-5 grid gap-3">
      {rows.map(([label, fact]) => (
        <div className="rounded-md bg-[#fbfcfb] p-3" key={`${broker.id}-${label}`}>
...
export async function generateMetadata({
  params,
}: BrokerComparisonPageProps): Promise<Metadata> {
  ...
  return {
    title,
    description:
      "A source-backed broker comparison foundation using structured broker facts.",
    alternates: {
    ...
    <div className="min-h-screen bg-[var(--background)]">
      <header className="border-b border-[var(--border)] bg-[var(--panel)]">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-5 lg:px-8">
          ...
          <Link className="text-sm font-medium text-[#5f6268]" href="/admin/brokers">
            Broker admin
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-6 py-10 lg:px-8">
        ...
        <section className="border-b border-[#d9ded7] pb-7">
          ...
          <p className="mt-5 max-w-3xl text-lg leading-8 text-[#5f6268]">
            Compare structured broker facts that have source fields attached.
            Empty cells mean no sourced data has been entered yet.
          </p>
        </section>

        <section className="mt-8 grid gap-4 md:grid-cols-2">
          {comparison.brokers.map((broker) => (
            <div className="rounded-md border border-[#d9ded7] bg-white p-5" key={broker.id}>
              ...
              <div className="mt-5">
                <AffiliateCta
                  ...
                  language={comparison.market.languageCode}
                  market={comparison.market.code}
                >
                  {`Visit ${broker.name}`}
                </AffiliateCta>
              </div>
          ...
          {categories.length === 0 ? (
            <div className="p-6">
              ...
              <p className="mt-2 text-sm leading-6 text-[#5f6268]">
                Add BrokerFact rows in Broker Manager to populate this
                comparison.
              </p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full border-collapse text-left text-sm">
                ...
                <tbody className="divide-y divide-[#eef1ed]">
                  {categories.map((item) => (
                    <tr className="align-top" key={item.category}>
